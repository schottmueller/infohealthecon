...
* News
- Due to heavy traffic, the provider currently disabled the downloads for the screencasts. For now, you can find them as well as the datasets for the empirical exercises [[https://uni-koeln.sciebo.de/s/yr6TZMJkZc05OeB][here]]. Please, download only once and not repeatedly (no new screencasts were added!).   
- The lecture on January 25 is moved from room H161 to room H162.  
- The lecture on December 21 will be substituted by a screencast due to health reasons. Screencast on [[https://web.tresorit.com/l/uSbeb#Bn1klbA__jMZOu5pS-cZ-g][out of sample predictions]] and on [[https://web.tresorit.com/l/bshVo#abOzSC3SUmHkaJAFw72nlQ][utilization management]] are now available. For the case study on the deductible, see  [[https://github.com/schottmueller/infohealthecon/blob/master/data/eigenRisico.org][here]].
- The lecture on December 14 is canceled due to an illness.  
#  - You are allowed to use calculators in the exam if these calculators (i) cannot store text, (ii) are not graphical and (iii) cannot solve equations for unknown variables. Put differently, your calculator should be able to do basic arithmetic of real numbers (addition, multiplication, subtraction, division and possibly also exponentiation, taking roots and logarithms as well as evaluating trigonometric functions) and nothing more.
  - Exam dates are Feb 1st, 10:15 (H161, 213/01/1.15) and March 20, 12:00 (Aula 1). Registration is open on KLIPS. We will try to grade the exams of the first date before the registration deadline of the second but we cannot guarantee that we will manage (we managed in previous years).
