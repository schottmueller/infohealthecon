...
With the RAND estimate of -0.2 we would then expect that the average expected cost of 10 year olds would increase by 0.2*30.9%=6.18%. Our actual (deflated) cost increase is significantly higher but at least it has roughly the same order of magnitude. But as I mentioned above we are comparing apples and oranges here.
#+BEGIN_SRC julia :results output :exports both :session
