...
3. Health insurance plans have often can be described by a deductible $D$, a copayment rate $c$ and a maximal out of pocket amount $M$: Up to $D$ all expenditures are paid by the insured, for every $ spent between $D$ and $M$ the insured pays $c$ and the insurance bears all expenses above $M$. [fn:: Hence, the total copayment if expenditures are $x$ is $x$ if $x\leq D$; is $D+c(x-D)$ if $D<x<M$ and is $D+cM$ for $x\geq M$] Assume that consumers act as to maximize the utility function $cons-0.5(2-s-t)^2$ where $cons$ is consumption, i.e. all money left to the consumer after paying for treatment $t\in[0,2-s]$, and $s\leq2$ is a health state. Assume that the consumer has an initial wealth of 4 (net of the insurance premium) and therefore consumption is $4-t$.
  1. Suppose the consumer has no insurance (or equivalently $D>4$). How much treatment will he buy in health state $s\in[0,2]$?
  2. Suppose the consumer has a coinsurance rate of $c\in[0,1)$ while $D=0$ and $M=\infty$. How much treatment will he buy in health state $s\in[0,2]$?
  3. Now let $D=0.5$, $c=1/2$ and $M=\infty$. How much treatment will the consumer buy in health state $s\in[0,2]$?
  4. (Extra exercise not covered in class: Verify that the consumer chooses either $t=0$ or $t=2-s$ if in the previous question $M=1$.)
  5. Think now about expected expenditure at the time of insurance purchase (i.e. we do not know the health state yet). Under which conditions on the distribution of health states will an increase in the deductible reduce expected expenditures? What does this imply for the effectiveness of small deductibles in reducing expected expenditures?
...
# E3.3: MB=2-s-t while MC either 1,c or 0. If s\geq 1.5, then MC=1 and MB<1 an clearly t=0. For s\in[1,1.5) spending t>0.5 is possible but not optimal as t_c<1/2; hence t=0 for all s\in[1,2].  For $s<1$ compare t_1=0 and t_c=1.5-s  which lead to utility u_1= 4-.5(2-s)^2=2+2s-.5s^2 and u_c=4-(.5+ (1-s)/2)-(.5)^2=2.75+s/2 and u_0=4-(.5+.5*.5)=3.25; where $u_1>u_c$ iff s>2.5-\sqrt{4.75}=0.32. 
# E3.4: For s>0.32, the previous exercise verifies. For s<0.32 compare $t_c$ and t_0=2-s leading to u_0=4-(.5.+0.5/2)=3.25.  As 3.25>u_c for s<0.32, the result follows.  
# E3.5: Say we increase from D_1 to D_2. Then expenditures are only affected if health states in which we want to spend between D_1 and D_2 (under deductible D_1) have positive probability. Otherwise there is no difference between D_1 and D_2. This implies that small deductibles have hardly an effect as they can prevent only small expenditures and have no effect on big spenders that cause the majority of the health care expenditures.
